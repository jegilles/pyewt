...
params = {
    ...
    "reg": "none", # options: none, gaussian, average
...
ewtv, mfb, maxima, vorpartition, plane = pyewt.ewt2d_watershed(f,params)

# Plot the detected scale-space maxima on the Fourier spectrum
if plot_maxima:
    pyewt.plot_scalespace_maxima(f,maxima,radius=5)

# plot the Fourier supports delineated by the Voronoi cells boundaries
if plot_bounds:
    pyewt.show_ewt2d_watershed_boundaries(f, vorpartition, logspec=1)
...
if plot_inverse:
    inv = pyewt.iewt2d_watershed(ewtv, mfb)
